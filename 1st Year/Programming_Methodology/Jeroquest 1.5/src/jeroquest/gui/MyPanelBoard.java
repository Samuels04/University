...
public class MyPanelBoard extends JLayeredPane {
	...
	private void initialiseCharacters() {
		...
		DynamicVectorCharacters chars = this.theGame.getCharacters();

		// recreate the characters
		for (int i = 0; i < chars.length(); i++) {
			if (chars.get(i).getPosition() != null) {
				// a JLabel is created the icon
				JLabel lab = new MyJLabelCharacter(chars.get(i));
				map.put(chars.get(i), lab);
