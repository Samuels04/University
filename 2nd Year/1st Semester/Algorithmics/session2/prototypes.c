...
int main() {
  ...
  // STUDENT: COMMENT/UNCOMMENT THE NEXT printf ACCORDING TO THE MEASURES YOU
  // NEED TO TAKE
  ...
  for (i = 0; i < NUM_SIZES; i++) {
    ...
    printf("\t\t%d\t\t%f", sizes[i],(time_fin1-time_ini1)
    /(double)CLOCKS_PER_SEC / REPSEARCH); printf("\t\t%f\n",
    (time_fin2-time_ini2) /(double)CLOCKS_PER_SEC / REPSEARCH);

    */

    // SNIPPET OF CODE TO ANALYSE THE ADDITION OF THE ELEMENTS IN THE DIAGONAL
    // OF THE MATRIX
    ...
    if (AddRecurResult != AddIterResult) {
      printf("Inconsistency in the addition\n\n");
      return -1;
    }

    printf("\t\t%d\t\t%f", sizes[i],
           (time_fin1 - time_ini1) / (double)CLOCKS_PER_SEC / REPITERA);
