// ---------------------------------------------------------------------------------
//
//  ALGORITHMICS: IMPLEMENTATION ACCORDING TO THE BACKTRACKING SCHEMA DE LA
//  SOLUCI�N DEL PROBLEMA DE
//              DESCOMPOSE A NUMBER N INTO M ADDENDS WITH N>=M>0, WHERE THE
//              ADDITION OF THE M ADDENDS HAS TO BE N AND THEIR PRODUCT IS
//              MAXIMUM. IT IS ALSO GENERATED: ALL FEASIBLE SOLUTIONS AND ONE
//              FEASIBLE SOLUTION.
//
//
...
// prototypes of auxiliary functions to be used in backtracking: correct, value,
// process
...
int main() {
  ...
  // Pull up the solution (secuence of optimal decisions and the maximum
  // benefit)
  process(x_best, M);
...
// definitions for auxiliary functions to be used in backtracking: correct,
// value and process
int add(int *x, int k) {
  int sum = 0;
  int i = 0;
...
void DESCOMPOSE_BACK_OPTIMAL(int N, int M, int k, int *x, int *x_best,
                             int *v_best) {
  int i, aux;
  x[k] = 0;
  while (x[k] < N) {
