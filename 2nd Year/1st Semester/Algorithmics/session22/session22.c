...
int main() {
  ...
  printf(" --------------------");
  printf("\n\n The max common divisor of %d and %d is: %d  (recursive "
         "version)\n\n",
         a, b, MCD_recursive(a, b));
  printf("\n\n The max common divisor of %d and %d is: %d  (iterative "
         "version)\n\n",
  ...
  printf(" BOARDS\n");
  printf(" --------");
  printf("\n\n The number of ways to place a %d x %d board on a %d x %d board "
         "is: %d (recursive version)\n\n",
         m, m, n, n, BOARDS_recursive(n, m));
  printf("\n\n The number of ways to place a %d x %d board on a %d x %d board "
         "is: %d (iterative version) \n\n",
  ...
  printf(" ADD VECTOR\n");
  printf(" -----------");
  printf("\n\n The sum of the elements of the vector is: %d (recursive "
         "backward version)\n\n",
         iADD_VECTOR_recursive_backward(V, ELEMENTS));
  printf("\n\n The sum of the elements of the vector is: %d  (iterative "
         "backward version)\n\n",
  ...
  /*
    ...
    palindrom  (recursive version)\n\n");
    //if (PALINDROME_VECTOR_iterative(W,1,ELEMENTS)) printf("\n\n The vector is
    palindrome  (iterative version) \n\n");
    ...
    SYMMETRIC (recursive version)\n\n"); else printf("\n\n THE MATRIX IS NOT
    SYMMETRIC (recursive version)\n\n");

    //if (SYMMETRIC_MATRIX_iterative(M,ORDER))  printf("\n\n THE MATRIX IS
    SYMMETRIC (iterative version)\n\n");
...
int POWER_recursive(int a, int n) {
  if (n == 0)
    ...
    return POWER_recursive(a, n - 1) * a;
}
...
int MCD_recursive(int a, int b) {
  if (a == b)
    ...
    return MCD_recursive(a - b, b);
  else
    return MCD_recursive(a, b - a);
}
